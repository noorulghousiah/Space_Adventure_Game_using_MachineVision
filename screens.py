...
def game_screen(screen, current_screen):
    ...
    imgBalloon = pygame.image.load('Resources/star.png').convert_alpha()
    # Resize the image to a smaller size, e.g., 40x40 pixels
    imgBalloon = pygame.transform.scale(imgBalloon, (40, 40))
    spaceship_img = pygame.image.load('Resources/ufo.png').convert_alpha()
    spaceship_img = pygame.transform.scale(spaceship_img, (50, 50))  # Resize spaceship image as needed


    imgJumpscare = pygame.image.load('Resources/HD-wallpaper-cute-puppy-lovely-adorable-sweet-dog-face-cute-graphy-paws-puppies-face-eyes-animals-dogs-puppy-dog.jpg').convert_alpha()
    imgJumpscare = pygame.transform.scale(imgJumpscare, (width, height))

    bgMusic = pygame.mixer.Sound('Resources/Elevator Music (Kevin MacLeod) - Background Music (HD).mp3')
    jumpscareSound = pygame.mixer.Sound('Resources/Sad Trombone - Sound Effect (HD).mp3')
